...
export const calculateMetricsService = async (rango) => {
    let dateQuery = {};
    const now = new Date();
    ...
    const [statsResult, topProductosResult, topMarcasResult, vipUsersResult] = await Promise.all([
        ...
        Order.aggregate([
            { $match: baseMatch },
            { $unwind: "$products" },
            { $group: { _id: "$products.producto", ventas: { $sum: "$products.cantidad" } } },
            { $sort: { ventas: -1 } },
            { $limit: 5 },
        ...
        Order.aggregate([
            { $match: baseMatch },
            { $unwind: "$products" },
            { $lookup: { from: "products", localField: "products.producto", foreignField: "_id", as: "prod" } },
            { $unwind: "$prod" },
            { $group: { _id: "$prod.marca", ventas: { $sum: "$products.cantidad" } } },
            { $sort: { ventas: -1 } },
            { $limit: 3 }
        ]),
        Order.aggregate([
    ...
    const coloresTacticos = ["#FF5E00", "#D6FF00", "#00A8FF", "#8C92AC", "#2C2C2C"];

    const topProductosConColor = topProductosResult.map((prod, index) => ({
        ...prod,
        color: coloresTacticos[index] || "#000"
    }));
